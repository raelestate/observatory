...
function updateAPOD() {
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      const hours = Math.floor((timeToNextApod % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
      const minutes = Math.floor((timeToNextApod % (1000 * 60 * 60)) / (1000 * 60));
      const seconds = Math.floor((timeToNextApod % (1000 * 60)) / 1000);
      document.getElementById('time-to-next-apod').innerText = `Time to next APOD: ${hours}h`;
      
      // Add event listener for the "View Full Resolution" button
      const fullResolutionButton = document.getElementById('view-full-resolution');
